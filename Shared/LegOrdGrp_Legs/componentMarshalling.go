...
func (m *LegOrdGrp_Legs) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if len(m.LegAllocs) > 0 {
    
    (*res).WriteString("670=")
    (*res).WriteString(strconv.Itoa(len(m.LegAllocs)))
    (*res).WriteString("\x01")
...
func (m *LegOrdGrp_Legs) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message LegOrdGrp_Legs (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *LegOrdGrp_Legs) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 670:
            // This counter (NoLegAllocs) indicates the start of repeated group block 
            count, _ := strconv.Atoi(value)
            m.LegAllocs = make([]LegPreAllocGrp_LegAllocs, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
                m._controlBlock.mostRecentRepeatingGroup[i] = &m.LegAllocs[i]
            }
            m._controlBlock.mostRecentRepeatingGroupCounter = 0
            used = true
