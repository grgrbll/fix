...
func (m *InstrmtMDReqGrp_RelatedSym) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *InstrmtMDReqGrp_RelatedSym) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message InstrmtMDReqGrp_RelatedSym (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *InstrmtMDReqGrp_RelatedSym) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
