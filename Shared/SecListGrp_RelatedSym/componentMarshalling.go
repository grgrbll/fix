...
func (m *SecListGrp_RelatedSym) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *SecListGrp_RelatedSym) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message SecListGrp_RelatedSym (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *SecListGrp_RelatedSym) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 555:
            ...
            count, _ := strconv.Atoi(value)
            m.Legs = make([]InstrmtLegSecListGrp_Legs, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
