...
func (m *QuotCxlEntriesGrp_QuoteEntries) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *QuotCxlEntriesGrp_QuoteEntries) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message QuotCxlEntriesGrp_QuoteEntries (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *QuotCxlEntriesGrp_QuoteEntries) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
