...
func (m *QuotEntryAckGrp_QuoteEntries) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.QuoteEntryRejectReason.HasValue() {
        (*res).WriteString("368=")
        val, err := m.QuoteEntryRejectReason.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *QuotEntryAckGrp_QuoteEntries) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message QuotEntryAckGrp_QuoteEntries (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *QuotEntryAckGrp_QuoteEntries) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 368:
            if !m.QuoteEntryRejectReason.HasValue() {
                used = true
                err = m.QuoteEntryRejectReason.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
