...
func (m *UnderlyingStipulations_UnderlyingStips) MarshalFIX(res *bufio.Writer) error {
    var err error
    
    if m.UnderlyingStipType.HasValue() {
        (*res).WriteString("888=")
        val, err := m.UnderlyingStipType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.UnderlyingStipValue.HasValue() {
        (*res).WriteString("889=")
        val, err := m.UnderlyingStipValue.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *UnderlyingStipulations_UnderlyingStips) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message UnderlyingStipulations_UnderlyingStips (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *UnderlyingStipulations_UnderlyingStips) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 888:
            if !m.UnderlyingStipType.HasValue() {
                used = true
                err = m.UnderlyingStipType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 889:
            if !m.UnderlyingStipValue.HasValue() {
                used = true
                err = m.UnderlyingStipValue.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
