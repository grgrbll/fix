...
func (m *ListOrdGrp_Orders) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.ListSeqNo.HasValue() {
        (*res).WriteString("67=")
        val, err := m.ListSeqNo.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *ListOrdGrp_Orders) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message ListOrdGrp_Orders (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *ListOrdGrp_Orders) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 67:
            if !m.ListSeqNo.HasValue() {
                used = true
                err = m.ListSeqNo.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
