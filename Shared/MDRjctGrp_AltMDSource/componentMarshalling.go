...
func (m *MDRjctGrp_AltMDSource) MarshalFIX(res *bufio.Writer) error {
    var err error
    
    if m.AltMDSourceID.HasValue() {
        (*res).WriteString("817=")
        val, err := m.AltMDSourceID.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *MDRjctGrp_AltMDSource) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message MDRjctGrp_AltMDSource (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *MDRjctGrp_AltMDSource) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 817:
            if !m.AltMDSourceID.HasValue() {
                used = true
                err = m.AltMDSourceID.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
