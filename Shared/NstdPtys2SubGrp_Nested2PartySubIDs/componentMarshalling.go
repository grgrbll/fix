...
func (m *NstdPtys2SubGrp_Nested2PartySubIDs) MarshalFIX(res *bufio.Writer) error {
    var err error
    
    if m.Nested2PartySubID.HasValue() {
        (*res).WriteString("760=")
        val, err := m.Nested2PartySubID.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.Nested2PartySubIDType.HasValue() {
        (*res).WriteString("807=")
        val, err := m.Nested2PartySubIDType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *NstdPtys2SubGrp_Nested2PartySubIDs) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message NstdPtys2SubGrp_Nested2PartySubIDs (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *NstdPtys2SubGrp_Nested2PartySubIDs) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 760:
            if !m.Nested2PartySubID.HasValue() {
                used = true
                err = m.Nested2PartySubID.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 807:
            if !m.Nested2PartySubIDType.HasValue() {
                used = true
                err = m.Nested2PartySubIDType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
