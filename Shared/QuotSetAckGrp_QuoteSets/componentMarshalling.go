...
func (m *QuotSetAckGrp_QuoteSets) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *QuotSetAckGrp_QuoteSets) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message QuotSetAckGrp_QuoteSets (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *QuotSetAckGrp_QuoteSets) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 295:
            ...
            count, _ := strconv.Atoi(value)
            m.QuoteEntries = make([]QuotEntryAckGrp_QuoteEntries, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
