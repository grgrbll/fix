...
func (m *PosUndInstrmtGrp_Underlyings) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.UnderlyingSettlPriceType.HasValue() {
        (*res).WriteString("733=")
        val, err := m.UnderlyingSettlPriceType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if len(m.UnderlyingAmounts) > 0 {
    
    (*res).WriteString("984=")
    (*res).WriteString(strconv.Itoa(len(m.UnderlyingAmounts)))
    (*res).WriteString("\x01")
    ...
    if m.UnderlyingDeliveryAmount.HasValue() {
        (*res).WriteString("1037=")
        val, err := m.UnderlyingDeliveryAmount.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *PosUndInstrmtGrp_Underlyings) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message PosUndInstrmtGrp_Underlyings (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *PosUndInstrmtGrp_Underlyings) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 733:
            if !m.UnderlyingSettlPriceType.HasValue() {
                used = true
                err = m.UnderlyingSettlPriceType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 984:
            // This counter (NoUnderlyingAmounts) indicates the start of repeated group block 
            count, _ := strconv.Atoi(value)
            m.UnderlyingAmounts = make([]UnderlyingAmount_UnderlyingAmounts, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
                m._controlBlock.mostRecentRepeatingGroup[i] = &m.UnderlyingAmounts[i]
            }
            m._controlBlock.mostRecentRepeatingGroupCounter = 0
            used = true
        ...
        case 1037:
            if !m.UnderlyingDeliveryAmount.HasValue() {
                used = true
                err = m.UnderlyingDeliveryAmount.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
