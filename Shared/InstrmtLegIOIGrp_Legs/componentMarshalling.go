...
func (m *InstrmtLegIOIGrp_Legs) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.LegIOIQty.HasValue() {
        (*res).WriteString("682=")
        val, err := m.LegIOIQty.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *InstrmtLegIOIGrp_Legs) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message InstrmtLegIOIGrp_Legs (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *InstrmtLegIOIGrp_Legs) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 682:
            if !m.LegIOIQty.HasValue() {
                used = true
                err = m.LegIOIQty.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
