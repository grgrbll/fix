...
func (m *CollInqQualGrp_CollInquiryQualifier) MarshalFIX(res *bufio.Writer) error {
    var err error
    
    if m.CollInquiryQualifier.HasValue() {
        (*res).WriteString("896=")
        val, err := m.CollInquiryQualifier.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *CollInqQualGrp_CollInquiryQualifier) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message CollInqQualGrp_CollInquiryQualifier (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *CollInqQualGrp_CollInquiryQualifier) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 896:
            if !m.CollInquiryQualifier.HasValue() {
                used = true
                err = m.CollInquiryQualifier.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
