...
func (m *ExpirationQty_Expiration) MarshalFIX(res *bufio.Writer) error {
    var err error
    
    if m.ExpType.HasValue() {
        (*res).WriteString("982=")
        val, err := m.ExpType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.ExpQty.HasValue() {
        (*res).WriteString("983=")
        val, err := m.ExpQty.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *ExpirationQty_Expiration) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message ExpirationQty_Expiration (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *ExpirationQty_Expiration) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 982:
            if !m.ExpType.HasValue() {
                used = true
                err = m.ExpType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 983:
            if !m.ExpQty.HasValue() {
                used = true
                err = m.ExpQty.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
