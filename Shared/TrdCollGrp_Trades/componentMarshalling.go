...
func (m *TrdCollGrp_Trades) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *TrdCollGrp_Trades) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message TrdCollGrp_Trades (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *TrdCollGrp_Trades) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
