...
func (m *RoutingGrp_RoutingIDs) MarshalFIX(res *bufio.Writer) error {
    var err error
    
    if m.RoutingType.HasValue() {
        (*res).WriteString("216=")
        val, err := m.RoutingType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.RoutingID.HasValue() {
        (*res).WriteString("217=")
        val, err := m.RoutingID.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *RoutingGrp_RoutingIDs) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message RoutingGrp_RoutingIDs (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *RoutingGrp_RoutingIDs) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 216:
            if !m.RoutingType.HasValue() {
                used = true
                err = m.RoutingType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 217:
            if !m.RoutingID.HasValue() {
                used = true
                err = m.RoutingID.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
