...
func (m *TradeCaptureReportRequest_Legs) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *TradeCaptureReportRequest_Legs) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message TradeCaptureReportRequest_Legs (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *TradeCaptureReportRequest_Legs) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
