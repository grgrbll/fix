...
func (m *IOIQualGrp_IOIQualifiers) MarshalFIX(res *bufio.Writer) error {
    var err error
    
    if m.IOIQualifier.HasValue() {
        (*res).WriteString("104=")
        val, err := m.IOIQualifier.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *IOIQualGrp_IOIQualifiers) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message IOIQualGrp_IOIQualifiers (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *IOIQualGrp_IOIQualifiers) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 104:
            if !m.IOIQualifier.HasValue() {
                used = true
                err = m.IOIQualifier.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
