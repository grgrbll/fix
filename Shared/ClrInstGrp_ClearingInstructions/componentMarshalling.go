...
func (m *ClrInstGrp_ClearingInstructions) MarshalFIX(res *bufio.Writer) error {
    var err error
    
    if m.ClearingInstruction.HasValue() {
        (*res).WriteString("577=")
        val, err := m.ClearingInstruction.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *ClrInstGrp_ClearingInstructions) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message ClrInstGrp_ClearingInstructions (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *ClrInstGrp_ClearingInstructions) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 577:
            if !m.ClearingInstruction.HasValue() {
                used = true
                err = m.ClearingInstruction.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
