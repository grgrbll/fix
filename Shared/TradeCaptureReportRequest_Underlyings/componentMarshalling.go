...
func (m *TradeCaptureReportRequest_Underlyings) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *TradeCaptureReportRequest_Underlyings) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message TradeCaptureReportRequest_Underlyings (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *TradeCaptureReportRequest_Underlyings) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
