...
func (m *MassQuoteAcknowledgement) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.QuoteRejectReason.HasValue() {
        (*res).WriteString("300=")
        val, err := m.QuoteRejectReason.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if len(m.QuoteSets) > 0 {
    
    (*res).WriteString("296=")
    (*res).WriteString(strconv.Itoa(len(m.QuoteSets)))
    (*res).WriteString("\x01")
...
func (m *MassQuoteAcknowledgement) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message MassQuoteAcknowledgement (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *MassQuoteAcknowledgement) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 300:
            if !m.QuoteRejectReason.HasValue() {
                used = true
                err = m.QuoteRejectReason.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 296:
            // This counter (NoQuoteSets) indicates the start of repeated group block 
            count, _ := strconv.Atoi(value)
            m.QuoteSets = make([]QuotSetAckGrp_QuoteSets, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
                m._controlBlock.mostRecentRepeatingGroup[i] = &m.QuoteSets[i]
            }
            m._controlBlock.mostRecentRepeatingGroupCounter = 0
            used = true
