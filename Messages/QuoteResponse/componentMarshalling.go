...
func (m *QuoteResponse) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.QuoteRespType.HasValue() {
        (*res).WriteString("694=")
        val, err := m.QuoteRespType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *QuoteResponse) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message QuoteResponse (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *QuoteResponse) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 694:
            if !m.QuoteRespType.HasValue() {
                used = true
                err = m.QuoteRespType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 23:
            if !m.IOIID.HasValue() {
                used = true
                err = m.IOIID.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
