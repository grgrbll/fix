...
func (m *SecurityDefinitionRequest) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.SecurityRequestType.HasValue() {
        (*res).WriteString("321=")
        val, err := m.SecurityRequestType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *SecurityDefinitionRequest) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message SecurityDefinitionRequest (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *SecurityDefinitionRequest) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 321:
            if !m.SecurityRequestType.HasValue() {
                used = true
                err = m.SecurityRequestType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
