...
func (m *BidResponse) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *BidResponse) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message BidResponse (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *BidResponse) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 420:
            ...
            count, _ := strconv.Atoi(value)
            m.BidComponents = make([]BidCompRspGrp_BidComponents, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
