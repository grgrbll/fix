...
func (m *TradeCaptureReportRequest) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.TradeRequestType.HasValue() {
        (*res).WriteString("569=")
        val, err := m.TradeRequestType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    (*res).WriteString("580=")
    (*res).WriteString(strconv.Itoa(len(m.Dates)))
    (*res).WriteString("\x01")
...
func (m *TradeCaptureReportRequest) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message TradeCaptureReportRequest (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *TradeCaptureReportRequest) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 569:
            if !m.TradeRequestType.HasValue() {
                used = true
                err = m.TradeRequestType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 711:
            ...
            count, _ := strconv.Atoi(value)
            m.Underlyings = make([]TradeCaptureReportRequest_Underlyings, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
        ...
        case 555:
            ...
            count, _ := strconv.Atoi(value)
            m.Legs = make([]TradeCaptureReportRequest_Legs, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
        ...
        case 580:
            // This counter (NoDates) indicates the start of repeated group block 
            count, _ := strconv.Atoi(value)
            m.Dates = make([]TradeCaptureReportRequest_Dates, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
                m._controlBlock.mostRecentRepeatingGroup[i] = &m.Dates[i]
            }
            m._controlBlock.mostRecentRepeatingGroupCounter = 0
            used = true
