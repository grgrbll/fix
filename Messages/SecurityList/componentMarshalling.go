...
func (m *SecurityList) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *SecurityList) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message SecurityList (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *SecurityList) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 146:
            ...
            count, _ := strconv.Atoi(value)
            m.RelatedSym = make([]SecListGrp_RelatedSym, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
