...
func (m *SecurityListUpdateReport) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *SecurityListUpdateReport) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message SecurityListUpdateReport (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *SecurityListUpdateReport) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 146:
            ...
            count, _ := strconv.Atoi(value)
            m.RelatedSym = make([]SecLstUpdRelSymGrp_RelatedSym, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
