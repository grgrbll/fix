...
func (m *ContraryIntentionReport) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.ContIntRptID.HasValue() {
        (*res).WriteString("977=")
        val, err := m.ContIntRptID.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.LateIndicator.HasValue() {
        (*res).WriteString("978=")
        val, err := m.LateIndicator.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.InputSource.HasValue() {
        (*res).WriteString("979=")
        val, err := m.InputSource.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    (*res).WriteString("981=")
    (*res).WriteString(strconv.Itoa(len(m.Expiration)))
    (*res).WriteString("\x01")
...
func (m *ContraryIntentionReport) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message ContraryIntentionReport (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *ContraryIntentionReport) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 977:
            if !m.ContIntRptID.HasValue() {
                used = true
                err = m.ContIntRptID.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 978:
            if !m.LateIndicator.HasValue() {
                used = true
                err = m.LateIndicator.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 979:
            if !m.InputSource.HasValue() {
                used = true
                err = m.InputSource.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 981:
            // This counter (NoExpiration) indicates the start of repeated group block 
            count, _ := strconv.Atoi(value)
            m.Expiration = make([]ExpirationQty_Expiration, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
                m._controlBlock.mostRecentRepeatingGroup[i] = &m.Expiration[i]
            }
            m._controlBlock.mostRecentRepeatingGroupCounter = 0
            used = true
