...
func (m *DerivativeSecurityListRequest) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.SecurityListRequestType.HasValue() {
        (*res).WriteString("559=")
        val, err := m.SecurityListRequestType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *DerivativeSecurityListRequest) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message DerivativeSecurityListRequest (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *DerivativeSecurityListRequest) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 559:
            if !m.SecurityListRequestType.HasValue() {
                used = true
                err = m.SecurityListRequestType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
