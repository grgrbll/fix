...
func (m *Quote) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.BidSwapPoints.HasValue() {
        (*res).WriteString("1065=")
        val, err := m.BidSwapPoints.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.OfferSwapPoints.HasValue() {
        (*res).WriteString("1066=")
        val, err := m.OfferSwapPoints.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *Quote) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message Quote (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *Quote) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 555:
            ...
            count, _ := strconv.Atoi(value)
            m.Legs = make([]LegQuotGrp_Legs, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
        ...
        case 1065:
            if !m.BidSwapPoints.HasValue() {
                used = true
                err = m.BidSwapPoints.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 1066:
            if !m.OfferSwapPoints.HasValue() {
                used = true
                err = m.OfferSwapPoints.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
