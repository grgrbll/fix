...
func (m *CollateralInquiryAck) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.CollInquiryStatus.HasValue() {
        (*res).WriteString("945=")
        val, err := m.CollInquiryStatus.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.CollInquiryResult.HasValue() {
        (*res).WriteString("946=")
        val, err := m.CollInquiryResult.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if len(m.CollInquiryQualifier) > 0 {
    
    (*res).WriteString("938=")
    (*res).WriteString(strconv.Itoa(len(m.CollInquiryQualifier)))
    (*res).WriteString("\x01")
...
func (m *CollateralInquiryAck) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message CollateralInquiryAck (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *CollateralInquiryAck) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 945:
            if !m.CollInquiryStatus.HasValue() {
                used = true
                err = m.CollInquiryStatus.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 946:
            if !m.CollInquiryResult.HasValue() {
                used = true
                err = m.CollInquiryResult.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 938:
            // This counter (NoCollInquiryQualifier) indicates the start of repeated group block 
            count, _ := strconv.Atoi(value)
            m.CollInquiryQualifier = make([]CollInqQualGrp_CollInquiryQualifier, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
                m._controlBlock.mostRecentRepeatingGroup[i] = &m.CollInquiryQualifier[i]
            }
            m._controlBlock.mostRecentRepeatingGroupCounter = 0
            used = true
