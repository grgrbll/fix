...
func (m *OrderCancelRequest) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *OrderCancelRequest) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message OrderCancelRequest (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *OrderCancelRequest) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
