...
func (m *RequestForPositionsAck) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.PosReqStatus.HasValue() {
        (*res).WriteString("729=")
        val, err := m.PosReqStatus.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *RequestForPositionsAck) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message RequestForPositionsAck (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *RequestForPositionsAck) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 729:
            if !m.PosReqStatus.HasValue() {
                used = true
                err = m.PosReqStatus.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
