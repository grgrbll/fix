...
func (m *CrossOrderCancelRequest) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *CrossOrderCancelRequest) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message CrossOrderCancelRequest (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *CrossOrderCancelRequest) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 552:
            ...
            count, _ := strconv.Atoi(value)
            m.Sides = make([]SideCrossOrdCxlGrp_Sides, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
