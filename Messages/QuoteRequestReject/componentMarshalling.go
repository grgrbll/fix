...
func (m *QuoteRequestReject) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.RFQReqID.HasValue() {
        (*res).WriteString("644=")
        val, err := m.RFQReqID.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.QuoteRequestRejectReason.HasValue() {
        (*res).WriteString("658=")
        val, err := m.QuoteRequestRejectReason.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *QuoteRequestReject) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message QuoteRequestReject (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *QuoteRequestReject) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 644:
            if !m.RFQReqID.HasValue() {
                used = true
                err = m.RFQReqID.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 658:
            if !m.QuoteRequestRejectReason.HasValue() {
                used = true
                err = m.QuoteRequestRejectReason.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 146:
            ...
            count, _ := strconv.Atoi(value)
            m.RelatedSym = make([]QuotReqRjctGrp_RelatedSym, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
