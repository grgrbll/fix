...
func (m *MassQuote) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.DefBidSize.HasValue() {
        (*res).WriteString("293=")
        val, err := m.DefBidSize.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.DefOfferSize.HasValue() {
        (*res).WriteString("294=")
        val, err := m.DefOfferSize.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *MassQuote) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message MassQuote (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *MassQuote) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 293:
            if !m.DefBidSize.HasValue() {
                used = true
                err = m.DefBidSize.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 294:
            if !m.DefOfferSize.HasValue() {
                used = true
                err = m.DefOfferSize.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 296:
            ...
            count, _ := strconv.Atoi(value)
            m.QuoteSets = make([]QuotSetGrp_QuoteSets, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
