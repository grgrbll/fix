...
func (m *Heartbeat) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *Heartbeat) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message Heartbeat (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *Heartbeat) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
