...
func (m *RFQRequest) MarshalFIX(res *bufio.Writer) error {
    var err error
...
func (m *RFQRequest) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message RFQRequest (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *RFQRequest) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 146:
            ...
            count, _ := strconv.Atoi(value)
            m.RelatedSym = make([]RFQReqGrp_RelatedSym, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
