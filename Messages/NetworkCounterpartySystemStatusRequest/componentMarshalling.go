...
func (m *NetworkCounterpartySystemStatusRequest) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.NetworkRequestType.HasValue() {
        (*res).WriteString("935=")
        val, err := m.NetworkRequestType.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *NetworkCounterpartySystemStatusRequest) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message NetworkCounterpartySystemStatusRequest (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *NetworkCounterpartySystemStatusRequest) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 935:
            if !m.NetworkRequestType.HasValue() {
                used = true
                err = m.NetworkRequestType.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 936:
            ...
            count, _ := strconv.Atoi(value)
            m.CompIDs = make([]CompIDReqGrp_CompIDs, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
