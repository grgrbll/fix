...
func (m *TestRequest) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.TestReqID.HasValue() {
        (*res).WriteString("112=")
        val, err := m.TestReqID.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *TestRequest) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message TestRequest (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *TestRequest) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 112:
            if !m.TestReqID.HasValue() {
                used = true
                err = m.TestReqID.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
