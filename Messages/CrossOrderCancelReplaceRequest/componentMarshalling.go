...
func (m *CrossOrderCancelReplaceRequest) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.CrossPrioritization.HasValue() {
        (*res).WriteString("550=")
        val, err := m.CrossPrioritization.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *CrossOrderCancelReplaceRequest) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message CrossOrderCancelReplaceRequest (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *CrossOrderCancelReplaceRequest) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 550:
            if !m.CrossPrioritization.HasValue() {
                used = true
                err = m.CrossPrioritization.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 552:
            ...
            count, _ := strconv.Atoi(value)
            m.Sides = make([]SideCrossOrdModGrp_Sides, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
