...
func (m *CollateralAssignment) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.CollAsgnRefID.HasValue() {
        (*res).WriteString("907=")
        val, err := m.CollAsgnRefID.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *CollateralAssignment) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message CollateralAssignment (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *CollateralAssignment) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 907:
            if !m.CollAsgnRefID.HasValue() {
                used = true
                err = m.CollAsgnRefID.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
