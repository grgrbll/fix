...
func (m *SecurityTypes) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.TotNoSecurityTypes.HasValue() {
        (*res).WriteString("557=")
        val, err := m.TotNoSecurityTypes.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    (*res).WriteString("558=")
    (*res).WriteString(strconv.Itoa(len(m.SecurityTypes)))
    (*res).WriteString("\x01")
...
func (m *SecurityTypes) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message SecurityTypes (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *SecurityTypes) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 557:
            if !m.TotNoSecurityTypes.HasValue() {
                used = true
                err = m.TotNoSecurityTypes.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 558:
            // This counter (NoSecurityTypes) indicates the start of repeated group block 
            count, _ := strconv.Atoi(value)
            m.SecurityTypes = make([]SecTypesGrp_SecurityTypes, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
                m._controlBlock.mostRecentRepeatingGroup[i] = &m.SecurityTypes[i]
            }
            m._controlBlock.mostRecentRepeatingGroupCounter = 0
            used = true
