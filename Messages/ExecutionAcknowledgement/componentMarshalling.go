...
func (m *ExecutionAcknowledgement) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.ExecAckStatus.HasValue() {
        (*res).WriteString("1036=")
        val, err := m.ExecAckStatus.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if m.DKReason.HasValue() {
        (*res).WriteString("127=")
        val, err := m.DKReason.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
...
func (m *ExecutionAcknowledgement) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message ExecutionAcknowledgement (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *ExecutionAcknowledgement) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 1036:
            if !m.ExecAckStatus.HasValue() {
                used = true
                err = m.ExecAckStatus.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 127:
            if !m.DKReason.HasValue() {
                used = true
                err = m.DKReason.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
