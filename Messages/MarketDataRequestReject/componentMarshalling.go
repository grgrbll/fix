...
func (m *MarketDataRequestReject) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.MDReqRejReason.HasValue() {
        (*res).WriteString("281=")
        val, err := m.MDReqRejReason.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    if len(m.AltMDSource) > 0 {
    
    (*res).WriteString("816=")
    (*res).WriteString(strconv.Itoa(len(m.AltMDSource)))
    (*res).WriteString("\x01")
...
func (m *MarketDataRequestReject) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message MarketDataRequestReject (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *MarketDataRequestReject) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 281:
            if !m.MDReqRejReason.HasValue() {
                used = true
                err = m.MDReqRejReason.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 816:
            // This counter (NoAltMDSource) indicates the start of repeated group block 
            count, _ := strconv.Atoi(value)
            m.AltMDSource = make([]MDRjctGrp_AltMDSource, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
                m._controlBlock.mostRecentRepeatingGroup[i] = &m.AltMDSource[i]
            }
            m._controlBlock.mostRecentRepeatingGroupCounter = 0
            used = true
