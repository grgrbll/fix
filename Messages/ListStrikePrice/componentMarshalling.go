...
func (m *ListStrikePrice) MarshalFIX(res *bufio.Writer) error {
    var err error
    ...
    if m.TotNoStrikes.HasValue() {
        (*res).WriteString("422=")
        val, err := m.TotNoStrikes.MarshalFIX()
        if err != nil {
            return err
        }
        (*res).Write(val)
    ...
    (*res).WriteString("428=")
    (*res).WriteString(strconv.Itoa(len(m.Strikes)))
    (*res).WriteString("\x01")
...
func (m *ListStrikePrice) UnmarshalFIX(input io.Reader) error {
    var err error
    var field []byte
    for field, err = input.ReadSlice([]byte("\x01")); err == nil {
        ...
        if len(slices) != 2 {
            ...
            if !used {
                err = errors.New(fmt.Sprintf("Field unused by message ListStrikePrice (%s)", string(field)))
            }
        }
        if err != nil {
            break
...
func (m *ListStrikePrice) UnmarshalFieldFIX(id int, value []byte) (bool, error) {
    ...
    if !used {
        ...
        case 422:
            if !m.TotNoStrikes.HasValue() {
                used = true
                err = m.TotNoStrikes.UnmarshalFIX(value)
                if err != nil {
                    return used, err
                }
            }
        ...
        case 428:
            // This counter (NoStrikes) indicates the start of repeated group block 
            count, _ := strconv.Atoi(value)
            m.Strikes = make([]InstrmtStrkPxGrp_Strikes, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
                m._controlBlock.mostRecentRepeatingGroup[i] = &m.Strikes[i]
            }
            m._controlBlock.mostRecentRepeatingGroupCounter = 0
            used = true
        ...
        case 711:
            ...
            count, _ := strconv.Atoi(value)
            m.Underlyings = make([]UndInstrmtStrkPxGrp_Underlyings, count)
            m._controlBlock.mostRecentRepeatingGroup = make([]FixMessage, count)
            for i := 0; i < count; i++ {
